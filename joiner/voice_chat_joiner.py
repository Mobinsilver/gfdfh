...
class VoiceChatJoiner:
    ...
    async def schedule_auto_leave(self, client, group_link, voice_chat_id=None):
        ...
        try:
            ...
            task_key = f"{phone}_{group_link}"
            
            # Cancel existing task if any
            if task_key in self.auto_leave_tasks:
                self.auto_leave_tasks[task_key].cancel()
            
            # Create new task
            task = asyncio.create_task(self._auto_leave_after_delay(client, group_link, voice_chat_id, phone))
            self.auto_leave_tasks[task_key] = task
    ...
    async def _auto_leave_after_delay(self, client, group_link, voice_chat_id, phone):
        ...
        try:
            ...
            logger.info(f"Auto leaving {phone} from {group_link} after {self.auto_leave_time} minutes")
            ...
            if group_link in self.joined_groups:
                self.joined_groups.remove(group_link)
            
            # Remove task from tracking
            task_key = f"{phone}_{group_link}"
            if task_key in self.auto_leave_tasks:
                del self.auto_leave_tasks[task_key]
            
            logger.info(f"Successfully auto left {phone} from {group_link}")
            
        except asyncio.CancelledError:
            logger.info(f"Auto leave task cancelled for {phone}")
        except Exception as e:
            logger.error(f"Error in auto leave for {phone}: {e}")
    
    def cancel_auto_leave(self, phone, group_link):
        """Cancel auto leave task for specific account and group"""
        task_key = f"{phone}_{group_link}"
        if task_key in self.auto_leave_tasks:
            self.auto_leave_tasks[task_key].cancel()
            del self.auto_leave_tasks[task_key]
            logger.info(f"Cancelled auto leave for {phone} from {group_link}")
    
    def get_auto_leave_time(self):
        """Get current auto leave time"""
        return self.auto_leave_time
    ...
    def add_account_to_voice_chat(self, phone, group_link, voice_chat_id):
        ...
        for vc in self.account_voice_chats[phone]:
            if vc['group_link'] == group_link and vc['voice_chat_id'] == voice_chat_id:
                return False  # Already in this voice chat
        ...
        logger.info(f"Account {phone} added to voice chat {group_link}")
        return True
    ...
    def remove_account_from_voice_chat(self, phone, group_link, voice_chat_id):
        ...
        # Find and remove the voice chat
        for i, vc in enumerate(self.account_voice_chats[phone]):
            if vc['group_link'] == group_link and vc['voice_chat_id'] == voice_chat_id:
                del self.account_voice_chats[phone][i]
                logger.info(f"Account {phone} removed from voice chat {group_link}")
                return True
        
        return False
    
    def get_account_voice_chats(self, phone):
        """Get all voice chats for a specific account"""
        return self.account_voice_chats.get(phone, [])
    
    def get_all_account_voice_chats(self):
        """Get all voice chats for all accounts"""
        return self.account_voice_chats
    
    def get_voice_chat_accounts(self, group_link, voice_chat_id):
        """Get all accounts in a specific voice chat"""
        accounts = []
        for phone, voice_chats in self.account_voice_chats.items():
            for vc in voice_chats:
                if vc['group_link'] == group_link and vc['voice_chat_id'] == voice_chat_id:
                    accounts.append(phone)
                    break
        return accounts
    
    def clear_account_voice_chats(self, phone):
        """Clear all voice chats for a specific account"""
        if phone in self.account_voice_chats:
            del self.account_voice_chats[phone]
            logger.info(f"Cleared all voice chats for account {phone}")
            return True
        return False
    
    async def join_multiple_voice_chats(self, voice_chat_configs):
        """
        Join multiple voice chats with different account distributions
        
        voice_chat_configs format:
        [
            {
                'group_link': '@group1',
                'voice_chat_id': 'voice_chat_id_1',
                'account_count': 10  # How many accounts to use for this voice chat
            },
            {
                'group_link': '@group2', 
                'voice_chat_id': 'voice_chat_id_2',
                'account_count': 15
            }
        ]
        """
        ...
        total_accounts_needed = sum(config['account_count'] for config in voice_chat_configs)
        if total_accounts_needed > len(active_clients):
            logger.warning(f"Not enough accounts. Need {total_accounts_needed}, have {len(active_clients)}")
            # Distribute accounts proportionally
            scale_factor = len(active_clients) / total_accounts_needed
            for config in voice_chat_configs:
                config['account_count'] = int(config['account_count'] * scale_factor)
        ...
        for i, config in enumerate(voice_chat_configs):
            ...
            # Get accounts for this voice chat
            accounts_for_this_chat = active_clients[account_index:account_index + account_count]
            ...
            # Delay between different voice chats
            if i < len(voice_chat_configs) - 1:
                await asyncio.sleep(5)
        
        logger.info(f"Multiple voice chat join completed. Results: {results}")
        return results
