...
class TelegramJoinerBot:
    ...
    async def start_bot(self):
        ...
        try:
            ...
            logger.info(f"Using API_ID: {temp_api_id}")
            logger.info(f"Using API_HASH: {temp_api_hash[:10]}...")
            
            self.bot = TelegramClient('bot_session', temp_api_id, temp_api_hash)
            logger.info("TelegramClient created successfully")
            
            await self.bot.start(bot_token=BOT_TOKEN)
            logger.info("Bot started with Telegram API")
...
/password رمز_عبور_جدید
    ...
    def is_authorized_user(self, user_id):
        ...
        logger.info(f"Authorization check for {user_id}: owner={is_owner}, admin={is_admin}, result={result}")
