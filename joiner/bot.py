    ...
    async def get_api_credentials(self, phone_number):
        ...
        try:
            ...
            try:
                ...
                api_credentials = await temp_client(functions.help.GetAppConfigRequest())
...
/addvoice شماره لینک ویس_چت - اضافه کردن اکانت به ویس چت
...
**نکته:** این دستور همه اکانت‌های فعال را به گروه/کانال مورد نظر جوین می‌کند.
    ...
    async def handle_phone_number(self, message):
        ...
        try:
            ...
            # Check if user is admin
            if sender_id not in self.admin_users:
                await message.reply("❌ شما دسترسی لازم را ندارید.")
                return
...
**نکته:** این دستور مالک ربات را تنظیم می‌کند.
            ...
            # Only allow if no owner is set yet
            if self.owner_id is not None:
                await message.reply("❌ مالک ربات قبلاً تنظیم شده است.")
                return
...
/help - راهنمای کامل
    ...
    async def handle_addvoice_command(self, message):
        """Handle /addvoice command - Add account to specific voice chat"""
        try:
            text = message.text
            sender_id = message.sender_id
            ...
            if len(text.split()) < 3:
                await message.reply("""
❌ فرمت صحیح: /addvoice شماره_تلفن لینک_گروه ویس_چت_آیدی

**مثال:**
/addvoice +989123456789 @group1 voice_chat_123

**نکته:** این دستور اکانت مشخص شده را به ویس چت اضافه می‌کند.
                """)
                return
            ...
            if not account_active:
                await message.reply(f"❌ اکانت `{phone}` غیرفعال است.")
                return
            
            # Check if account is already in this voice chat
            ...
            for vc in existing_voice_chats:
                if vc['group_link'] == group_link and vc['voice_chat_id'] == voice_chat_id:
                    await message.reply(f"❌ اکانت `{phone}` قبلاً در این ویس چت حضور دارد.")
                    return
            
            # Get client for the account
            if phone not in self.account_manager.clients:
                await message.reply(f"❌ اکانت `{phone}` متصل نیست.")
                return
            
            client = self.account_manager.clients[phone]
            
            await message.reply(f"🔄 در حال اضافه کردن اکانت `{phone}` به ویس چت...")
            
            # Join the group first
            group_result = await self.voice_chat_joiner.join_group(client, group_link)
            if not group_result:
                await message.reply(f"❌ خطا در جوین شدن به گروه `{group_link}`")
                return
            
            # Join the voice chat
            await asyncio.sleep(2)
            voice_result = await self.voice_chat_joiner.join_voice_chat(client, group_result, voice_chat_id)
            if not voice_result:
                await message.reply(f"❌ خطا در جوین شدن به ویس چت `{voice_chat_id}`")
                return
            
            # Add to tracking
            self.voice_chat_joiner.add_account_to_voice_chat(phone, group_link, voice_chat_id)
            
            # Schedule auto leave
            await self.voice_chat_joiner.schedule_auto_leave(client, group_link, voice_chat_id)
            
            await message.reply(f"""
✅ **اکانت با موفقیت به ویس چت اضافه شد!**
...
/listvoice - لیست همه ویس چت‌ها
            ...
            logger.info(f"Account {phone} added to voice chat {group_link} by {sender_id}")
...
🎤 **مدیریت ویس چت:**
`/addvoice +989123456789 @group voice_chat_id` - اضافه کردن اکانت به ویس چت
